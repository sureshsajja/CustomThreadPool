...
public interface ThreadPoolWithJobAffinity {
    ...
    /**
     * Execute a given job. The jobId determines the thread which executes the job.All the jobs submitted with
     * the same job Id should be executed by the same thread of the pool. Multiple jobs submitted for a given job id
